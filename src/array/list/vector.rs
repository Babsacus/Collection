        ...
        T: PartialEq + PartialOrd + Clone + Default + Copy + Debug,
{
    ...
    /// Resizes the 'vector' to the specified length with any new elements set to their default
    /// value.
        ...
        T: PartialEq + PartialOrd + Clone + Default + Copy + Debug,
{
    ...
    fn extend(&mut self, additional: usize) {
        self.arr.resize(self.arr.len() + additional, T::default());
    }
    ...
    /// value.
    fn resize(&mut self, length: usize) {
        self.arr.resize(length, T::default());
    }
